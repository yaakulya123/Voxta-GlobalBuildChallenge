...
// Map MediaPipe gesture names → ASL gloss tokens
const GESTURE_MAP = {
  Open_Palm:   ['HELLO'],
  Thumb_Up:    ['GOOD'],
  Thumb_Down:  ['BAD'],
...
const DEBOUNCE_MS = 1500   // min ms between same gesture firing again
const MIN_SCORE   = 0.72   // confidence threshold
...
export async function initGestureDetector() {
  ...
  console.log('[Gesture] MediaPipe GestureRecognizer ready')
}

/**
 * Run one frame of detection on a <video> element.
 * Returns an array of gloss tokens (may be empty).
 */
export function detectGesture(videoEl) {
