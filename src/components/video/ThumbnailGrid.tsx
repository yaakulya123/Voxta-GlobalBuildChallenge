...
export const ThumbnailGrid: React.FC<ThumbnailGridProps> = ({ isLocalVideoOn, isBlurOn, webcamRef, peerStream, peerInfo }) => {
  ...
  return (
    <div className="w-full h-32 md:h-40 flex gap-4 overflow-x-auto pb-2 custom-scrollbar">
      ...
      <div className="relative flex-shrink-0 w-48 md:w-64 h-full rounded-2xl overflow-hidden bg-panel-bg shadow-lg border border-primary-accent border-2">
        ...
        {isReady && isLocalVideoOn && (
           <div className="absolute top-2 right-2 flex items-center gap-1.5 bg-primary-accent/90 backdrop-blur-sm px-2 py-0.5 rounded-full shadow-[0_0_10px_rgba(59,130,246,0.6)] border border-white/20">
             <div className="w-1.5 h-1.5 bg-white rounded-full animate-ping" />
             <span className="text-[9px] font-bold tracking-wider text-white">AI PIPELINE</span>
           </div>
        )}
